...
// Adjusted Appointment interface to match what's being passed from DoctorDashboard
interface Appointment {
  id: string;
  doctorName: string; // Doctor's name (which is user.name from dashboard)
  date: string;       // Date of consultation (placeholder in dashboard)
  time: string;
  type: 'video' | 'audio' | 'in-person';
  symptoms: string; // Symptoms are available from the dashboard consultation object
}

interface ConsultationModalProps {
  isOpen: boolean;
  onClose: () => void;
  // Use the refined Appointment interface
  appointment: Appointment | null; 
  user: User; // This is the doctor's user object
  language: 'en' | 'hi' | 'pa' | 'ta'; // Added 'ta'
  isOnline: boolean;
}
...
export function ConsultationModal({ isOpen, onClose, appointment, user, language, isOnline }: ConsultationModalProps) {
  ...
  const translations = {
    ...
    ta: {
        ...
        waitingForDoctor: "மருத்துவர் இணைய காத்திருக்கிறது",
        connectionQuality: "இணைப்பு தரம்",
        duration: "கால அளவு",
        toggleVideo: "வீடியோ",
        toggleAudio: "ஆடியோ",
        endCall: "ஆலோசனையை முடிக்கவும்",
        switchToAudio: "ஆடியோ முறைக்கு மாறவும்",
        poorConnection: "இணைப்பு பலவீனமாக உள்ளது. ஆடியோ முறைக்கு மாறவும்.",
        doctorJoined: "மருத்துவர் இணைந்துள்ளார்",
      ...
      <DialogContent className="max-w-4xl h-[80vh]">
        <DialogHeader>
          <DialogTitle className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              {/* This will now show the patient's name */}
              <span>{t.consultation} - {appointment.id}</span> 
            </div>
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-4 h-full">
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-blue-900 to-purple-900 flex items-center justify-center">
                  {appointment.type === 'video' && videoEnabled ? (
                    <div className="text-center text-white">
                      ...
                      {/* Note: In a real scenario, this should show the patient's name, not doctorName */}
          ...
          <div className="space-y-4">
            {/* Doctor Info (Updated to display Doctor's info from the prop 'user') */}
            <Card>
            ...
            {/* Patient Symptoms - Added for relevance */}
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm">Patient Symptoms</CardTitle>
              </CardHeader>
